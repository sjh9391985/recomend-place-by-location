...
class DirectionServiceTest extends AbstractIntegrationContainerBaseTest{
    ...
    private PharmacySearchService pharmacySearchService = Mock() // Mock 객체로 생성
    ...
    def "buildDirectionList - 결과 값이 거리 순으로 정렬이 되는지 확인" () {
        ...
        double inputLongtitude = 127.037033003036

        def documentDto = DocumentDto.builder()
        .addressName(addressName)
        .latitude(inputLatitude)
        .longitude(inputLongtitude)
        .build()
        ...
        then:
        results.size() == 2
        results.get(0).targetPharmacyName == "호수이온누리약국"
